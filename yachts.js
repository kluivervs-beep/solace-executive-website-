...
// per broker PDF (see supabase-schema.sql). price_from is already our
// own marked-up rate (broker's raw rate is never stored or shown).
    ...
    <article class="yacht-card">
      ...
      <div class="yacht-card-body">
        ...
        <div class="yacht-card-footer">
          ${
            yacht.price_from
              ? `<span class="yacht-card-price"><span class="yacht-card-price-label">${t('yachts.from')}</span> ${formatPrice(yacht.price_from)} <span class="yacht-card-price-label">${t('yachts.perday')}</span></span>`
              : '<span></span>'
          }
...
async function load() {
  ...
  const { data, error } = await supabase
    .from('yachts')
    .select('name, model, length_m, guests_day, guests_night, cabins, base_harbour, price_from, photo_path')
    .eq('active', true)
